...
type Bucket interface {
	...
	GetFirstPair() Pair
	// Delete 会删除指定的键 - 元素对
	// 若调用此方法前已经锁定lock，则不要把lock传入！否则传入相应的lock!
	Delete(key string, lock sync.Locker) bool
	// Clear 会清空当前散列桶
	// 若在调用此方法前已经锁定，则不要把lock传入，否则必须传入对应的lock
	Clear(lock sync.Locker)
	// Size 会返回当前散列桶的尺寸
	Size() uint64
	// String 会返回当前散列桶的字符串表现形式
	String() string
}

// bukcet 代表并发安全的散列桶的类型
type bucket struct {
	// firstValue 存储的是键 - 元素对应列表的表头
	firstValue atomic.Value
	size       uint64
}

// 占位符
// 由于原子值不能存储nil,所以当散列桶空用此符占位
...
func (b *bucket) Delete(key string, lock sync.Locker) bool {
	...
	/*
		newFirstPair 为下一个Pair
		prePairs 的最后一个元素，是newFirstPair 前一个元素，现在需要连接起来
	*/
