...
func TestBucketPut(t *testing.T) {
	...
	for _, p := range testCases {
		...
		if ok {
			t.Fatalf("Coundn't put repeated pair to the bucket! (pair: %#v)", p)
		}
...
func TestBucketGetFirstPair(t *testing.T) {
	...
	for i := int(size - 1); i >= 0; i-- {
		...
		if current.Key() != expectedPair.Key() {
			t.Fatalf("Inconsistent key: expected: %s, actual: %s", expectedPair.Key(), current.Key())
		}

		if current.Element() != expectedPair.Element() {
			t.Fatalf("Inconsistent element: expected: %#v, actual: %#v", expectedPair.Element(), current.Element())
		}
