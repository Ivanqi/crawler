...
func TestCmapNew(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("No error when new a concurrent map with concurrency %d, but should not be the case!", concurrency)
	}
	...
	cm, err = NewConcurrentMap(concurrency, pairRedistributor)
	if err != nil {
		t.Fatalf("An error occurs when new a concurrent map: %s (concurrency: %d, pairRedistributor: %#v)", err, concurrency, pairRedistributor)
	}

	if cm == nil {
		t.Fatalf("Couldn't a new concurrent map! (concurrency: %d, pairRedistributor: %#v)", concurrency, pairRedistributor)
	}

	if cm.Concurrency() != concurrency {
		t.Fatalf("Inconsistent concurrency: expected: %d, actual: %d", concurrency, cm.Concurrency())
	}
}
