...
func New(mid module.MID, itemProcessors []module.ProcessItem, scoreCalculator module.CalculateScore) (module.Pipeline, error) {
	...
	if itemProcessors == nil {
		return nil, genParameterError("项目处理器列表为nil")
	}

	if len(itemProcessors) == 0 {
		return nil, genParameterError("项目处理器列表为空")
	}

	var innerProcessors []module.ProcessItem
	for i, pipeline := range itemProcessors {
		if pipeline == nil {
			err := genParameterError(fmt.Sprintf("nil 项目处理器[%d]", i))
			return nil, err
		}
...
// extraSummaryStruct 代表条目处理管道的额外信息的摘要类型
type extraSummaryStruct struct {
	FailFast        bool `json:"fail_fast"`
	ProcessorNumber int  `json:"processor_number"`
}

func (pipline *myPipeline) Summary() module.SummaryStruct {
	summary := pipline.ModuleInternal.Summary()
	summary.Extra = extraSummaryStruct{
		FailFast:        pipline.failFast,
		ProcessorNumber: len(pipline.itemProcessors),
	}
