...
// ParseResponse 代笔用于解析HTTP响应的函数的类型
...
type Module interface {
	...
	CalledCount() uint64
	// AcceptedCount 用于获取被当前组件接受的调用计数
	// 组件一般会由于超负荷或参数有误而拒绝调用
	AcceptedCount() uint64
	// CompletedCount 用于获取当前组件已成功完成的调用的计数
	CompletedCount() uint64
	// HandlingNumber 用于获取当前组件正在处理的调用数量
	HandlingNumber() uint64
	// Counts 用于一次性获取所有计数
	Counts() Counts
	// Summary 用于获取组件的摘要
...
type Downloader interface {
	...
	Download(reg *Request) (*Response, error)
}
...
type Pipeline interface {
	...
	// 这里的快速失败是指：只要在处理某个条目时在某一个步骤上出错，那么条目处理管道就会忽略掉后续的所有处理步骤并报告错误
	FailFast() bool
	// 设置是否快速失败
