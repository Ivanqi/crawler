...
func TestCheckStatus(t *testing.T) {
	...
	for _, currentStatus := range currentStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("它仍然可以检查当前状态不正确的状态 %q", GetStatusDescription(currentStatus))
		}
	}
	...
	for _, wantedStatus := range wantedStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("它仍然可以检查不正确的想要的状态 %q的状态", GetStatusDescription(wantedStatus))
		}
	}
	...
	for _, wantedStatus := range wantedStatusList {
		if err := checkStatus(currentStatus, wantedStatus, nil); err == nil {
			t.Fatalf("它仍然可以检查当前状态 %q 和 想要状态 %q", GetStatusDescription(currentStatus), GetStatusDescription(wantedStatus))
		}
	}

	wantedStatus = SCHED_STATUS_INITIALIZING
	if err := checkStatus(currentStatus, wantedStatus, nil); err != nil {
		t.Fatalf("检查状态出错: %s (currentStatus: %q, wantedStatus: %q)!", err, GetStatusDescription(currentStatus), GetStatusDescription(wantedStatus))
	}
...
func TestGetStatusDescription(t *testing.T) {
	...
	for status, expectedDesc := range statusMap {
		...
		if desc != expectedDesc {
			t.Fatalf("状态 %d的描述不一致。预期: %s, 实际: %s", status, expectedDesc, desc)
		}
	}
}
