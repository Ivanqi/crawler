...
func TestSchedInit(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("No error when initialize scheduler with illegal request arguments %+v!", invalidRequestArgs)
	}
	...
	if err == nil {
		t.Fatalf("No error when initialize scheduler with illegal data arguments %+v!", invalidDataArgs)
	}
	...
	for _, invalidModuleArgs := range invalidModuleArgsList {
		...
		if err == nil {
			t.Fatalf("No error when initialize scheduler with illegal module arguments %s!", invalidModuleArgs)
		}
	}
...
func TestSchedSendReq(t *testing.T) {
	...
	if urlMapLen != 1 {
		t.Fatalf("Inconsistent URL map length: expected: %d, actual: %d", 1, urlMapLen)
	}
	...
	if !mySched.sendReq(req) {
		t.Fatalf("Couldn't send request! (request: %#v)", req)
	}
